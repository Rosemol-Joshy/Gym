...
async function apiRequest(endpoint, options = {}) {
    try {
        ...
        if (err.message !== "Database Offline" && err.message !== "Session expired. Please login again.") {
...
async function fetchDashboardData() {
    try {
        ...
        if (data.isMockDb) {
            toggleDbWarning(true, "The backend could not connect to MySQL. Running in Sandbox Mock Mode with an in-memory database. All features (Check-in, Registration, timetables, billing payments) are fully functional, but changes will reset when the server restarts.");
        } else {
            toggleDbWarning(false);
        }
...
function renderSvgChart() {
    ...
    days.forEach((day, index) => {
        ...
        if (todayData[index] > 0) {
            ...
            currentPoints.push({ x, y: todayY });
        }
    });
    
    const avgPathD = buildSplinePath(avgPoints);
    const currentPathD = buildSplinePath(currentPoints);
    
    const bottomY = paddingY + graphHeight;
    const avgAreaD = avgPoints.length > 0 
        ? `${avgPathD} L ${avgPoints[avgPoints.length - 1].x} ${bottomY} L ${avgPoints[0].x} ${bottomY} Z`
        : "";
    const currentAreaD = currentPoints.length > 0
        ? `${currentPathD} L ${currentPoints[currentPoints.length - 1].x} ${bottomY} L ${currentPoints[0].x} ${bottomY} Z`
        : "";
    ...
    days.forEach((day, index) => {
        if (todayData[index] > 0) {
            ...
            currentDotsHtml += `
                <circle class="chart-dot" cx="${x}" cy="${y}" r="4.5" />
            `;
        }
    });
    ...
    container.innerHTML = `
        <svg class="chart-svg" width="100%" height="${containerHeight}">
            <defs>
                <linearGradient id="chartGradientToday" x1="0" y1="0" x2="0" y2="1">
                    <stop offset="0%" stop-color="var(--accent-crimson)" stop-opacity="0.25" />
                    <stop offset="100%" stop-color="var(--accent-crimson)" stop-opacity="0.0" />
                </linearGradient>
                <linearGradient id="chartGradientAvg" x1="0" y1="0" x2="0" y2="1">
                    <stop offset="0%" stop-color="rgba(142, 142, 147, 0.2)" stop-opacity="0.12" />
                    <stop offset="100%" stop-color="rgba(142, 142, 147, 0.2)" stop-opacity="0.0" />
                </linearGradient>
            </defs>
            ${gridLinesHtml}
            ${daysHtml}
            ${avgAreaD ? `<path d="${avgAreaD}" fill="url(#chartGradientAvg)" />` : ""}
            ${currentAreaD ? `<path d="${currentAreaD}" fill="url(#chartGradientToday)" />` : ""}
...
// ==========================================================================
// 9. AUTHENTICATION LOGIC & EVENT HANDLERS
// ==========================================================================
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    // Initial Authentication check & render
    checkAuthentication();
});
